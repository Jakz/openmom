...
SpellBookView::SpellBookView(ViewManager* gvm) : View(gvm), book(new BookView(6)), combatMode(false)
{
  ...
  buttons[NEXT_PAGE] = new SimpleButton("Next page", 29, 14, SpriteInfo(TextureID::SPELL_BOOK_CORNERS, 1, 0));
...
void SpellBookView::drawPage(u16 index)
{
  ...
  Fonts::drawString(i18n::s(page->getTitle()), FontFace::BROWN_SERIF, topLeftX[index]+52, topLeftY[index]-16, ALIGN_CENTER);
  ...
  for (int i = 0; i < 6; ++i)
  {
    ...
    if (entry.spell)
    {
      ...
      {
        clip = Fonts::drawString(i18n::s(spell->name), FontFace::BROWN_SMALL, topLeftX[index], curY, ALIGN_LEFT);
        clip2 = Fonts::drawString(Fonts::format("%d MP",g->spellMechanics.actualManaCost(player, spell, combatMode)), FontFace::BROWN_SMALL, topLeftX[index]+ROW_WIDTH, curY, ALIGN_RIGHT);
      }
      else
      {
        clip = Fonts::drawString(i18n::s(spell->name), FontFace::YELLOW_SMALL, topLeftX[index], curY, ALIGN_LEFT, &FontMap::Small::WHITE_GREY_BLINK);
        clip2 = Fonts::drawString(Fonts::format("%d MP",g->spellMechanics.actualManaCost(player, spell, combatMode)), FontFace::YELLOW_SMALL, topLeftX[index]+ROW_WIDTH, curY, ALIGN_RIGHT, &FontMap::Small::WHITE_GREY_BLINK);
      }
      ...
      else
      {
        ...
        int w = 0;
        if (!combatMode)
          w = Fonts::drawString("Instant", FontFace::BROWN_SMALL, topLeftX[index]+9, curY+7, ALIGN_LEFT);
