...
namespace skills
{
  ...
  template<typename T, typename S> void findAllEffectsOftype(const S& src, T& effects, UnitEffectType type)
  {
  ...
  template<typename S> bool hasSimpleEffect(const S& src, SimpleEffect::Type type)
  {
    for (const Skill* skill : src)
      for (const auto e : skill->getEffects())
        if (e->type == UnitEffectType::ABILITY && e->as<SimpleEffect>()->effect == type)
          return true;
